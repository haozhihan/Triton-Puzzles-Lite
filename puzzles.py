...
def add_mask2_kernel(x_ptr, z_ptr, N0, B0: tl.constexpr):
    ...
    offsets = tl.arange(0, B0) + block_id * B0
    x = tl.load(x_ptr + offsets, offsets < N0)
    tl.store(z_ptr + offsets, x + 10, offsets < N0)
    # Finish me!
    return
...
def sum_kernel(x_ptr, z_ptr, N0, N1, T, B0: tl.constexpr, B1: tl.constexpr):
    # Finish me!
    block_id = tl.program_id(0)
    off_thread = block_id * B0 + tl.arange(0, B0)
    mask_thread = off_thread < N0

    z = tl.zero([B0], dtype=tl.float32)

    for id_j in tl.range(0, T, B1):
        off_j = id_j + tl.arange(0, B1)
        off_ij = off_thread[:, None] * T + off_j[None, :] 
        mask_j = off_j < T
        mask_ij = mask_thread[:, None] & mask_j[None, :]

        x = tl.load(x_ptr + off_ij, mask_ij)
        z += tl.sum(x, axis=1)

    tl.store(z_ptr + off_thread, z, mask=mask_thread)
